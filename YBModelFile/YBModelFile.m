...
- (void)creatWithPath:(NSString *)path  {
    if (!path) {
        ...
        if (!bundle){
            NSAssert(0, @"自动获取桌面路径失败，请尝试添加文件路径");
            [WHToast showMessage:@"JSON数据错误" duration:1 finishHandler:nil];
        } ;
    ...
    //创建一个工具工作空间
    NSString *rootDirectoryPath = [self creatDirectoryWithPath:path.copy directoryName:@"CreatModel" cover:YES];
...
- (void)creatFilesWithDirectoryPath:(NSString *)path {
    switch (self.config.filePartitionMode) {
        ...
        case YBMFFilePartitionModeTogether: {
            ...
            NSString *resultStr = [NSString stringWithFormat:@"创建成功\n文件地址:\n %@",path];
            [WHToast showMessage:resultStr duration:5 finishHandler:nil];
        }
        default:
            break;
    }
